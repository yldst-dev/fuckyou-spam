...
| `ALLOWED_CHAT_IDS` | 봇이 작동할 그룹 ID (쉼표로 구분) | - | 필수 |
...
1. **그룹 ID 확인**: 봇을 그룹에 추가하고 `/chatid` 명령어로 그룹 ID를 확인하세요
2. **화이트리스트 설정**: 확인한 그룹 ID를 `.env` 파일의 `ALLOWED_CHAT_IDS`에 추가하세요
3. **봇 재시작**: 환경변수 변경 후 봇을 재시작하세요
4. 봇에게 **관리자 권한**을 부여하세요
5. **메시지 삭제** 권한이 반드시 필요합니다

⚠️ **보안 기능**: 봇은 `ALLOWED_CHAT_IDS`에 등록된 그룹에서만 작동하며, 허용되지 않은 그룹에서는 자동으로 나갑니다.

### 봇 명령어

- `/start` - 봇 소개 및 사용법 안내
- `/status` - 봇 상태 및 큐 대기 메시지 수 확인
- `/chatid` - 현재 그룹의 채팅 ID 확인 (화이트리스트 설정용)

### 자동 기능

- 그룹 메시지를 실시간으로 모니터링
- **비멤버 메시지 우선 검사**: 그룹에 참여하지 않은 사용자의 메시지를 우선적으로 분석
...
- 스팸으로 분류된 메시지 자동 삭제
- **상세한 삭제 알림**: 사용자 정보, 시각, 메시지 내용을 포함한 영구 경고 메시지
...
**비멤버 메시지 특별 검열:**
- 그룹 멤버가 아닌 사용자의 텔레그램 그룹/채널 초대 링크
- 비멤버의 의심스러운 홍보성 콘텐츠
- 비멤버의 사기 시도
...
### 🚨 스팸 삭제 알림 시스템
스팸 메시지가 삭제되면 그룹에 상세한 알림 메시지가 표시됩니다:
